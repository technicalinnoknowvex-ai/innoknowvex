...
const TechStarter = () => {
  ...
  const getMentorPlansCount = () => {
    return Object.values(selectedPlans).filter(plan => plan === PLAN_TYPES.MENTOR).length;
  };
  ...
  const isSelectionComplete = () => {
    return getSelectedCoursesCount() === MAX_COURSES && getMentorPlansCount() >= MIN_MENTOR_PLANS;
  };
  ...
  const handleAddToCart = (program, plan, price, actualPrice) => {
    ...
    if (plan !== PLAN_TYPES.MENTOR && !canSelectNonMentorPlan(program.id)) {
      toast.error(`You must select at least ${MIN_MENTOR_PLANS} Mentor plans! Only ${MAX_COURSES - MIN_MENTOR_PLANS} non-Mentor plans allowed.`, {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: false,
    ...
    let existingCart = JSON.parse(sessionStorage.getItem("techStarterCart")) || [];
    ...
    const packageInfo = {
      ...
      originalPrice: existingCart.reduce((sum, item) => sum + (item.actualPrice || item.price), 0),
      items: existingCart,
      coursesCount: existingCart.length,
      mentorPlansCount: existingCart.filter(i => i.plan === PLAN_TYPES.MENTOR).length
    };
  ...
  const discount = totalActual > 0 ? Math.round(((totalActual - FIXED_PACKAGE_PRICE) / totalActual) * 100) : 0;

  const renderPlanCard = (program, planType, planName, currentPrice, actualPrice) => {
    const planDiscount = Math.round(((actualPrice - currentPrice) / actualPrice) * 100);
    ...
    const isNonMentorDisabled = planName !== PLAN_TYPES.MENTOR && !canSelectNonMentorPlan(program.id);
    
    return (
      <div 
        className={`${style.planCard} ${isSelected ? style.selectedCard : ''} ${(isDisabled || isNonMentorDisabled) ? style.disabledCard : ''}`} 
      ...
      >
        <div className={style.planImageSection}>
          ...
          <div className={style.courseTitle}>
            <svg className={style.starIcon} width="20" height="20" viewBox="0 0 136 148" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className={style.planDetailsSection}>
          ...
          {isSelected ? (
            ...
            <button 
              ...
              onClick={() => handleAddToCart(program, planName, currentPrice, actualPrice)}
              disabled={isDisabled || !canSelect || isNonMentorDisabled}
              style={{
                opacity: (isDisabled || !canSelect || isNonMentorDisabled) ? 0.5 : 1,
                cursor: (isDisabled || !canSelect || isNonMentorDisabled) ? 'not-allowed' : 'pointer'
              }}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                <g fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2">
              ...
              {isDisabled ? 'Another Plan Selected' : !canSelect ? 'Max Limit Reached' : isNonMentorDisabled ? 'Need More Mentor Plans' : 'Add to Cart'}
            </button>
          )}
        </div>
  ...
  return (
    <div className={style.packsContainer}>
      <div className={style.headDiv}>
        <svg className={style.headstar1} width="60" height="60" viewBox="0 0 136 148" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <svg className={style.headstar2} width="60" height="60" viewBox="0 0 136 148" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M66.0962 1.74792C66.3511 -0.582641 69.4658 -0.582641 69.7207 1.74792L71.8573 21.2992C74.6162 46.5452 92.9484 66.4498 116.2 69.4453L134.207 71.7651C136.354 72.0419 136.354 75.4237 134.207 75.7005L116.2 78.0203C92.9484 81.0159 74.6162 100.92 71.8573 126.166L69.7207 145.717C69.4658 148.048 66.3511 148.048 66.0962 145.717L63.9596 126.166C61.2007 100.92 42.8685 81.0159 19.6167 78.0203L1.60985 75.7005C-0.536616 75.4237 -0.536616 72.0419 1.60985 71.7651L19.6167 69.4453C42.8685 66.4498 61.2007 46.5452 63.9596 21.2992L66.0962 1.74792Z" fill="#9F8310" />
        </svg>
      </div>

      <div className={style.packDescription}>
        <p>Select exactly <strong>4 courses</strong> from our collection of {techCourses.length} technology courses. <strong>At least 2 courses must have the Mentor plan</strong> selected. Get the complete package for just <strong>₹25,000</strong>!</p>
        {cartItems.length === MAX_COURSES && isSelectionComplete() && (
          <div className={style.priceRange}>
            <span className={style.priceLabel}>Your Total Package Price:</span>
            <span className={style.originalPrice}>₹{formatPrice(totalActual)}</span>
            <span className={style.arrow}>→</span>
            <span className={style.currentPrice}>₹{formatPrice(FIXED_PACKAGE_PRICE)}</span>
      ...
      <div className={style.selectionCounter}>
        ...
        <p className={style.mentorCount}>Mentor Plans: <strong>{getMentorPlansCount()}</strong> / {MIN_MENTOR_PLANS} minimum required</p>
        {getSelectedCoursesCount() === MAX_COURSES && getMentorPlansCount() >= MIN_MENTOR_PLANS ? (
          <div className={style.completeBadge}>
            <CheckCircle size={20} />
            <span>Selection Complete! Package Price: ₹{formatPrice(FIXED_PACKAGE_PRICE)}</span>
          </div>
        ) : getSelectedCoursesCount() === MAX_COURSES && getMentorPlansCount() < MIN_MENTOR_PLANS ? (
          <div className={style.warningBadge}>
            <AlertCircle size={20} />
            <span>Need {MIN_MENTOR_PLANS - getMentorPlansCount()} more Mentor plan(s)!</span>
          </div>
        ) : null}
      </div>
          ...
          {techCourses.map(program => {
            ...
            return (
              <React.Fragment key={program.id}>
                {renderPlanCard(program, 'self', PLAN_TYPES.SELF, coursePrice.self_current_price, coursePrice.self_actual_price)}
                {renderPlanCard(program, 'mentor', PLAN_TYPES.MENTOR, coursePrice.mentor_current_price, coursePrice.mentor_actual_price)}
                {renderPlanCard(program, 'professional', PLAN_TYPES.PROFESSIONAL, coursePrice.professional_current_price, coursePrice.professional_actual_price)}
              </React.Fragment>
            );
          })}
        </div>
