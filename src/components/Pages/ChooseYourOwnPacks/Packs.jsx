...
const Packs = () => {
  ...
  const [programsPrice, setProgramsPrice] = useState({}); // store as object for easy lookup
  const [loading, setLoading] = useState(false);
  const [priceLoadingStates, setPriceLoadingStates] = useState({}); // track loading for each course
  ...
  // fetch price per course from Supabase API
  const fetchProgramPrice = async (tag) => {
    try {
  ...
  useEffect(() => {
    ...
    // fetch each program separately from Supabase
  ...
  return (
    <>
      ...
      <div className={style.courses}>
        {Object.values(programs).map((item) => {
          const coursePrice = programsPrice[item.price_search_tag];
          const isPriceLoading = priceLoadingStates[item.price_search_tag];
              ...
              {isPriceLoading ? (
                ...
                <div className={style.coursePlan}>
                  <div className={style.priceDiv}>
                    ...
                    <h4 onClick={() => handleSelectedPlan(item.id, item.title, "Self", coursePrice.self_current_price, item.image)} className={style.pricetag} id='self'>Self</h4>
                    <div className={style.price}>₹{formatPrice(coursePrice.self_current_price)}</div>
                    <div className={style.actualprice}>₹{formatPrice(coursePrice.self_actual_price)}</div>
                    <div className={style.line}></div>
                  </div>
                  ...
                  <div className={style.priceDiv}>
                    ...
                    <h4 onClick={() => handleSelectedPlan(item.id, item.title, "Mentor", coursePrice.mentor_current_price, item.image)} className={style.pricetag} id='mentor'>Mentor</h4>
                    <div className={style.price}>₹{formatPrice(coursePrice.mentor_current_price)}</div>
                    <div className={style.actualprice}>₹{formatPrice(coursePrice.mentor_actual_price)}</div>
                    <div className={style.line}></div>
                  </div>
                  ...
                  <div className={style.priceDiv}>
                    ...
                    <h4 onClick={() => handleSelectedPlan(item.id, item.title, "Professional", coursePrice.professional_current_price, item.image)} className={style.pricetag} id='professional'>Professional</h4>
                    <div className={style.price}>₹{formatPrice(coursePrice.professional_current_price)}</div>
                    <div className={style.actualprice}>₹{formatPrice(coursePrice.professional_actual_price)}</div>
                    <div className={style.line}></div>
                  </div>
                </div>
              ) : (
                <div className={style.coursePlan}>
                  <div className={style.priceError}>
                    <p>Unable to load pricing</p>
                    <button 
                      onClick={() => fetchProgramPrice(item.price_search_tag)}
