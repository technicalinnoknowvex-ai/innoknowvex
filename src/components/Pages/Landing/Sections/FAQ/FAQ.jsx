...
const FAQ = () => {
  ...
  useEffect(() => {
    ...
    const el = answerTextRefs.current[activeFaq];
    if (!el) return;
    gsap.fromTo(
      el,
      { opacity: 0, y: 8 },
  ...
  useGSAP(
    () => {
      ...
      gsap.set(paraRef.current, { x: -40, opacity: 0 });
      // FAQ items: slide from right
      gsap.set(faqRefs.current, { x: 80, opacity: 0 });
      ...
      <div className={styles.sectionWrapper__innerContainer}>
        <section className={styles.leftSection}>
          <div className={styles.sectionHeadingContainer} ref={headingRef}>
            ...
            <h2 className={`${styles.sectionHeadingContainer__primaryHeading} gradientHeading`}>
              {heading}
            </h2>
        ...
        <section className={styles.rightSection}>
          {faqs.map((faq, fIndex) => (
            <div
              ...
              className={`${styles.faqWrapper} ${activeFaq === fIndex && styles["faqWrapper--active"]
                }`}
            ...
            >
              ...
              <div
                ...
                className={`${styles.answerWrapper} ${activeFaq === fIndex && styles["answerWrapper--active"]
                  }`}
                style={{
                  maxHeight:
                    activeFaq === fIndex
              ...
              >
                <p ref={(el) => (answerTextRefs.current[fIndex] = el)}>{faq.answer}</p>
              </div>
            </div>
          ))}
        </section>
