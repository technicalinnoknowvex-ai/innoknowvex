    ...
    items: [
      {
        ...
        hasSubMenu: true,
        subItems: [
          { label: "Online", href: "/programs/web-development?mode=online" },
          { label: "Offline", href: "/offline-courses/web-development" },
        ],
      },
    ...
    items: [
      {
        ...
        href: "/programs/machine-learning",
        hasSubMenu: true,
        subItems: [
          { label: "Online", href: "/programs/machine-learning?mode=online" },
          { label: "Offline", href: "/offline-courses/machine-learning" },
        ],
      },
      {
        ...
        hasSubMenu: true,
        subItems: [
          { label: "Online", href: "/programs/artificial-intelligence?mode=online" },
          { label: "Offline", href: "/offline-courses/artificial-intelligence" },
        ],
      },
      {
        label: "Data Science",
        ...
        hasSubMenu: true,
        subItems: [
          { label: "Online", href: "/programs/data-science?mode=online" },
          { label: "Offline", href: "/offline-courses/data-science" },
        ],
      },
      {
...
const navLinks = [
  ...
  { label: "Careers", type: "section", href: "/careers", sectionId: "careers" },
...
const Navbar = () => {
  ...
  const getNavLinkColor = () => {
    if (pathname.startsWith("/offline-courses")) {
      return "#f9fafb";
    }
    return navColor;
  };
  ...
  // Ensure all submenus are hidden on mount
  useEffect(() => {
    Object.values(subMenuRefs.current).forEach((subMenu) => {
      if (subMenu) {
  ...
  useGSAP(
    () => {
      ...
      // Initialize all sub-menus to hidden state
      Object.values(subMenuRefs.current).forEach((subMenu) => {
        if (subMenu) {
          gsap.set(subMenu, {
            autoAlpha: 0,
  ...
  const showSubMenu = (subMenuKey) => {
    const subMenu = subMenuRefs.current[subMenuKey];
    if (subMenu) {
      if (subMenuTimers.current[subMenuKey]) {
        clearTimeout(subMenuTimers.current[subMenuKey]);
      }

      // Hide all other submenus
      Object.entries(subMenuRefs.current).forEach(([key, otherSubMenu]) => {
        if (key !== subMenuKey && otherSubMenu) {
          gsap.to(otherSubMenu, {
            ...
            pointerEvents: "none",
            onComplete: () => {
              gsap.set(otherSubMenu, { display: "none" });
            },
          });
        }
      });
      ...
      gsap.set(subMenu, { display: "block", visibility: "visible" });
      gsap.to(subMenu, {
        autoAlpha: 1,
        x: 0,
        duration: 0.3,
  ...
  const hideSubMenu = (subMenuKey) => {
    const subMenu = subMenuRefs.current[subMenuKey];
    if (subMenu) {
      subMenuTimers.current[subMenuKey] = setTimeout(() => {
  ...
  const keepSubMenuOpen = (subMenuKey) => {
    if (subMenuTimers.current[subMenuKey]) {
      clearTimeout(subMenuTimers.current[subMenuKey]);
    }
  };
              ...
              <React.Fragment key={lIndex}>
                ...
                {link.type === "page" || link.type === "section" ? (
                  ...
                  >
                    ...
                    <p style={{ color: getNavLinkColor() }}>{link.label}</p>
                  </div>
                ) : link.type === "dropdown" ? (
                                ...
                                <>
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <div className={styles.programsList}>
                                        {category.items.map((item, iIndex) => {
                                          const subMenuKey = `${cIndex}-${iIndex}`;
                                          const hasSubMenu = item.hasSubMenu && item.subItems;
                                          
                                          return hasSubMenu ? (
                                            <div
                                              key={iIndex}
                                              className={styles.itemWithSubmenu}
                                            ...
                                            >
                                              ...
                                              <div
                                                ...
                                                onMouseEnter={() => keepSubMenuOpen(subMenuKey)}
                                                onMouseLeave={() => hideSubMenu(subMenuKey)}
                                              >
