...
export default function PlansSection({ courseName, isOffline }) {
  const searchParams = useSearchParams();
  const courseNameParam = courseName || searchParams.get("course") || "web-development";
  ...
  useEffect(() => {
    async function fetchPricingData() {
      try {
        ...
        console.log("Fetching pricing data for course:", courseNameParam);

        const response = await fetch(`/api/pricing/${courseNameParam}`, {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    ...
    if (autoOpen === "true" && plan && price) {
      ...
      // Clean URL params
      const newUrl = window.location.pathname + `?course=${courseNameParam}`;
      window.history.replaceState({}, "", newUrl);
    }
  }, [searchParams, courseNameParam]);
  ...
  const offlineActualPrice = 30000;
  const offlineCurrentPrice = 15000;

  if (loading) {
    return (
      <div
        id="plans-section"
        className={`${styles.plansContainer} ${isOffline ? styles.offlineStyle : ""}`}
      >
  ...
  return (
    ...
    >
      <PopUpForm
        ...
        onClose={closeForm}
        plan={selectedPlan}
        course={formatCourseName(courseNameParam)}
        price={selectedPlanPrice}
        courseId={pricingData?.course_name || courseNameParam}
      />

      <div className={`${styles.Plans} ${shouldAnimate ? styles.animateIn : ""}`}>
      ...
      <div className={styles.pricingCardLayout}>
        ...
        <div
          className={`${styles.pricingCards} ${
            isOffline ? styles.offlinePrimaryCard : ""
          } ${shouldAnimate ? styles.animateCard : ""}`}
          style={{ "--delay": "0s" }}
        >
          {isOffline ? (
            <div className={styles.offlineCardSplit}>
              <div className={styles.offlineLeftPane}>
                ...
                <p className={styles.price}>
                  ₹{formatPrice(offlineCurrentPrice)}
                  <span className={styles.originalPrice}>
                    ₹{formatPrice(offlineActualPrice)}
                  </span>
                </p>
                <p className={styles.planDesc}>
                  Classroom training, 1:1 mentorship, projects, and complete
                  career support in one offline program.
                </p>

                <div className={styles.offlineMiniVisual} aria-hidden="true">
                  <Image
                    src="/images/offline-classroom.png"
                    alt="Offline classroom learning illustration"
                    width={380}
                    height={180}
                    className={styles.offlineMiniImage}
                  />
                  <span>Offline classroom + live mentoring setup</span>
                </div>
              </div>

              <div className={styles.offlineRightPane}>
                <div className={styles.plancontent}>
                  {[
                    ...
                    <div key={label} className={styles.features}>
                      <Image
                ...
                <button
                  onClick={() => handleEnrollClick("Self", offlineCurrentPrice)}
            ...
            <>
              ...
              <p className={styles.planDesc}>
                Learn at your own pace with all the resources you need to
        ...
        {/* Additional plans kept for non-offline programs */}
        {!isOffline && (
          <>
        <div
          className={`${styles.pricingCards} ${shouldAnimate ? styles.animateCard : ""}`}
          style={{ "--delay": "0.15s" }}
        >
          <div className={styles.heading}>
            <svg
